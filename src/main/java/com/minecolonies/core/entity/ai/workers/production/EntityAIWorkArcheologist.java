...
import com.minecolonies.core.colony.buildings.workerbuildings.BuildingArcheologist;
import com.minecolonies.core.colony.jobs.JobArcheologist;
...
import static com.minecolonies.core.colony.buildings.modules.BuildingModules.ARCHEOLOGIST_MENU;
import static com.minecolonies.core.entity.ai.workers.production.EntityAIStructureMiner.*;

public class EntityAIWorkArcheologist extends AbstractEntityAICrafting<JobArcheologist, BuildingArcheologist>
{
    ...
    /**
     * Edibles that the worker will attempt to eat while on an expedition (unfiltered)
     */
    final List<ItemStack> expeditionEdible = IColonyManager.getInstance()
    ...
    public EntityAIWorkArcheologist(@NotNull JobArcheologist job)
    {
        super(job);
        super.registerTargets(
            new AITarget(ARCHEOLOGIST_EXPEDITION_LEAVE, this::leaveForExpedition, TICK_DELAY),
            new AITarget(ARCHEOLOGIST_EXPEDITION_AWAY, this::stayOnExpedition, TICK_DELAY),
            new AITarget(ARCHEOLOGIST_EXPEDITION_RETURN, this::returnFromExpedition, TICK_DELAY)
        );
    ...
    protected void updateRenderMetaData()
    {
        StringBuilder renderData = new StringBuilder(getState() == CRAFT
            || getState() == ARCHEOLOGIST_EXPEDITION_LEAVE
            || getState() == ARCHEOLOGIST_EXPEDITION_AWAY ? RENDER_META_WORKING : "");
    ...
    protected IAIState decide()
    {
        ...
        if (worker.getCitizenData().getColony().getTravelingManager().isTravelling(worker.getCitizenData()) || job.isOnExpedition())
        {
            return ARCHEOLOGIST_EXPEDITION_AWAY;
        }
        ...
        // This is async, so we could go on the expedition without it.
        ...
        // Check for materials needed to go on the expedition:
        IRecipeStorage rs = building.getFirstModuleOccurance(BuildingArcheologist.CraftingModule.class).getFirstRecipe(ItemStack::isEmpty);
        ...
        {
            final ICraftingBuildingModule module = building.getFirstModuleOccurance(BuildingArcheologist.CraftingModule.class);
    ...
    /**
     * Leave for the expedition by walking to the gate and going invisible.
     * For now this moves to the townhall and travels.
     */
    protected IAIState leaveForExpedition()
    {
        if (!worker.getInventoryCitizen().hasSpace())
        {
            return INVENTORY_FULL;
        ...
        //TODO: Replace with gates when these are introduced.
        final BlockPos portal = building.getColony().getBuildingManager().getTownHall().getPosition();
    ...
    /**
     * Stay "on the expedition" and process the queues
     */
    protected IAIState stayOnExpedition()
    {
        final ExpeditionLog expeditionLog = building.getFirstModuleOccurance(ExpeditionLogModule.class).getLog();
                ...
                if (currStack.getItem() instanceof ItemAdventureToken)
                {
                    if (currStack.hasTag())
                    {
                        ...
                        if (tag != null && tag.contains(TAG_DAMAGE))
                        {
                            ...
                            DamageSource source = world.damageSources().source(DamageSourceKeys.EXPEDITION);
                            ...
                            for (int hit = 0; mobHealth > 0 && !worker.isDeadOrDying(); hit++)
                            {
                                ...
                                boolean takeDamage = worker.getRandom().nextBoolean() && MineColonies.getConfig().getServer().archeologistWorkerTakesDamage.get();
        ...
        return ARCHEOLOGIST_EXPEDITION_RETURN;
    }
    ...
    private int xpOnDrop(Block block)
    {
        ...
        if (block == Blocks.SUSPICIOUS_GRAVEL || block == Blocks.SUSPICIOUS_GRAVEL)
        {
    ...
    /**
     * Return from the expedition by going visible, walking to building.
     */
    protected IAIState returnFromExpedition()
    {
        if (!walkToBuilding())
        {
            return getState();
    ...
    {
        final Set<ItemStorage> allowedItems = building.getModule(ARCHEOLOGIST_MENU).getMenu();
        expeditionEdible.removeIf(item -> allowedItems.contains(new ItemStorage(item)));
        return expeditionEdible;
    }
    ...
    {
        if (InventoryUtils.getItemCountInItemHandler(worker.getInventoryCitizen(), stack -> building.getModule(ARCHEOLOGIST_MENU).getMenu().contains(new ItemStorage(stack))) >= 16)
        {
            // We have enough food.
            return getState();
        }

        if (InventoryUtils.hasBuildingEnoughElseCount(building, stack -> building.getModule(ARCHEOLOGIST_MENU).getMenu().contains(new ItemStorage(stack)), 1) >= 1)
        {
            needsCurrently = new Tuple<>(stack -> building.getModule(ARCHEOLOGIST_MENU).getMenu().contains(new ItemStorage(stack)), 16);
