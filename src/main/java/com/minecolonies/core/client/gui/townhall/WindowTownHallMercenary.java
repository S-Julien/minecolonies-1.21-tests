...
public class WindowTownHallMercenary extends BOWindow implements ButtonHandler
{
    ...
    public WindowTownHallMercenary(final IColonyView c)
    {
        ...
        for (int i = 0; i < Math.min(amountOfMercenaries, 9); i++)
        {
            ...
            newImage.setImage(new ResourceLocation(
                    "minecolonies:textures/entity_icon/citizen/default/citizenmale3" + CitizenData.SUFFIXES.get(ColonyConstants.rand.nextInt(CitizenData.SUFFIXES.size())) + ".png"),
                false);
